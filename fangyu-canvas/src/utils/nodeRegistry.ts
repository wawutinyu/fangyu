  ...
  },
  {
    ...
    nodes: [
      ...
      },
      {
        ...
        outputSchema: [
          { name: 'result', type: 'any', label: '结论' },
          { name: 'success', type: 'boolean', label: '是否成功' },
          { name: 'turns', type: 'number', label: '轮次' },
        ],
        configSchema: [
          { key: 'model', label: '模型', type: 'select', default: 'deepseek-chat',
            options: ['deepseek-chat', 'deepseek-v4-flash', 'deepseek-v4-pro', 'deepseek-reasoner', 'gpt-4o', 'gpt-4o-mini', 'claude-3.5-sonnet'] },
          { key: 'toolbelt', label: '工具带', type: 'select', default: 'coding', options: ['coding', 'office'] },
          { key: 'max_turns', label: '最大轮次', type: 'number', default: 24, min: 2, max: 64 },
          { key: 'require_plan', label: '先 plan', type: 'select', default: true, options: [true, false] },
          { key: 'enable_task', label: '允许 task 子 Agent', type: 'select', default: true, options: [true, false] },
          { key: 'agent_mode', label: '模式', type: 'select', default: 'build', options: ['build', 'plan'] },
          { key: 'shell_policy', label: 'Shell 策略', type: 'select', default: 'ask', options: ['ask', 'allow', 'deny'] },
          { key: 'temperature', label: '温度', type: 'number', default: 0.2, min: 0, max: 2, step: 0.1 },
          { key: 'max_tokens', label: '最大 Token', type: 'number', default: 4096, min: 256, max: 128000 },
          { key: 'system', label: '系统提示（空则用 coding 默认）', type: 'textarea', default: '', placeholder: '留空使用 CODING_SYSTEM', rows: 4 },
        ],
      },
      {
