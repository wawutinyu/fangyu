...
export default function ChatInterface({ headerless }: ChatInterfaceProps) {
  ...
  }, [selectedAgent, selectedSkill, agents, agentNodes, agentCollabMode, collabPipeline])
    ...
    <>
      <div style={{ display: 'flex', gap: 4, padding: '4px 12px', borderBottom: '1px solid var(--border-light)', flexShrink: 0, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        {chatMode === 'agent' && agents.length > 0 && !agentCollabMode && (
          <>
            <select value={selectedAgent || agents[0]?.id || ''} onChange={e => { setSelectedAgent(e.target.value); setSelectedSkill('') }}
              style={{ marginLeft: 4, fontSize: 11, padding: '1px 4px', border: '1px solid #ddd', borderRadius: 4 }}>
              {agents.map(a => <option key={a.id} value={a.id}>{a.label}</option>)}
            </select>
            {(() => {
              const ag = agents.find(a => a.id === (selectedAgent || agents[0]?.id))
              const skills = ag?.agentCard?.skills || []
              if (skills.length <= 1) return null
              return (
                <select value={selectedSkill || skills[0]?.id || ''} onChange={e => setSelectedSkill(e.target.value)}
                  style={{ marginLeft: 4, fontSize: 11, padding: '1px 4px', border: '1px solid #ddd', borderRadius: 4 }}>
