...
def _run_flow(flow_config: dict):
    ...
    try:
        from app.routers.flow import run_flow_endpoint
        import asyncio
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        # Note: quick stub - real execution uses run_flow
        print(f"[trigger] Running flow: {json.dumps(flow_config, indent=2)[:200]}")
