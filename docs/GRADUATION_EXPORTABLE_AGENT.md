...
| 2 | **运行态**：`/orchestrate` / A2A 管线可跑通本厂+外来 | 部分（有 API / demo，非办公场景默认） |
| 3 | **导出态**：多 Agent 拓扑 **打进 Bundle / 部署物**，脱离 Studio 也能编 | ☐ 关键缺口 |
| 4 | 与 G2-A/B 结合：一句办公任务可拆给多专家；IM 可触发整网而非单人 | ☐ |
| 5 | 与 G2-C 结合：编排边上的权限（谁可调谁、跨 Agent 工具边界） | ☐ |

**现成资产（加速 G2-E，不代替毕业）：** Agent 画布 · `POST /orchestrate` · `intent/to-agents` · A2A · `a2a_multi_agent_demo.py`
...
G2-E 多编排   ████░░░░░░  ~35%（画布/A2A 有，导出+办公/IM 联通无）
────────────────────────
总毕业(G1∧G2) ██░░░░░░░░  ~20%
```

G2-E 是相对最接近的一块；**总毕业仍卡在导出态编排 + IM + 权限 + 办公成品**。

---

## 建议推进顺序（仍服务总毕业）

1. **收口 G1**（配 Key 跑绿 live）  
2. **G2-A workbuddy profile + 成品落盘**  
3. **G2-E 导出态多 Agent**（把画布拓扑打进 Bundle，复用现有 orchestrate）— **差异化最快**  
4. **G2-B 一条 IM** 触发单人 → 再触发整网  
5. **G2-D 托管面** · **G2-C 企业权限**（编排边 ACL 跟进）
