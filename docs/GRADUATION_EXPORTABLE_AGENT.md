...
| 3 | 可选云托管或本机「托管感」面板（用户能当服务用，不是一次性脚本） | 部分（CLI + `/api/v1/managed`；Studio 面板 ☐） |
...
```text
G1 OpenCode  █████████░  ~90%（live 已绿；人手绑仓可选）
G2-A 办公竖切 ██████░░░░  ~55%（profile+md+live；缺 office 格式）
G2-B IM       █████░░░░░  ~45%（飞书通道代码齐；真机暂缓）
G2-C 企业权限 ██░░░░░░░░  ~15%
G2-D 托管     ███████░░░  ~65%（manage 启停/日志/健康；缺 Studio 面板/升级）
G2-E 多编排   ██████░░░░  ~55%（导出态已通；权限/产品化不足）
────────────────────────
总毕业(G1∧G2) ████░░░░░░  ~40%
```
...
1. ~~收口 G1~~ ✅  
2. ~~G2-A / G2-E 骨架~~ ✅  
3. ~~G2-B 飞书通道代码~~ ✅（**真机订阅暂缓**）  
4. ~~G2-D 托管面~~ ✅（CLI + API；Studio 面板以后）  
5. **G2-C 企业权限** — 下一优先  
6. G2-A docx/xlsx；飞书真机 / Studio 托管面板以后再说  
