...
### 4.3 编排边 ACL（Agent→Agent）

与组织 ACL（人→资源）独立：topology 边上声明 **谁可调谁**。

- 模块：`core/topology_acl.py`
- 默认无声明 → 放行；`allowed_callers` / `deny_callers` / `edge_acl.rules` 命中则 fail-closed
- 审计事件：`edge_acl_violation`
- 挂载点：Bundle `orchestrate` 波次、`AgentOrchestrator.run_pipeline`、`AgentBus`（metadata 带 topology）

详见 [拓扑与 task](TOPOLOGY_AND_TASK.md#编排边-aclp2)。
